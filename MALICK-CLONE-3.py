...
uid = os.getuid()
key1 = open('/data/data/com.termux/files/usr/bin/.mrakingxxx-cov', 'r').read()
kex=(f"MALICK-XD~CREATE:{uid}TS{key1}110E==")
...
------------------------------------------
 Author : MALICK-XD
 Github : https://github.com/LEGEND-MALICK (Follow Must)
 Tools  : Auto Facebook Register
 Contact: +91 8266803073
 Version: 1.6
\033[1;37m------------------------------------------""") 
try:
    clear()
    ...
    if "Version-1.5" in response:
        ...
        if kex in response:
            ...
            linex()
            print(" You cannot run this tool without MALICK permeations \n Note: Agar Tool mai Koi Masla Aata HA To Me Jald Se Jald Try Kronga Fix Krne Ke Agr Nhi Hota To May Be kujh time lag jya fix krne me!\n payment krne ka bad return nhi hoge agr buy krna ha to ok else skip,exit")
            linex()
            print(" Your key: "+fkeyx);subprocess.check_output(["am", "start", "https://api.whatsapp.com/send?phone=+918266803073&text="+(" Hi MALICK sir i want Buy Auto Create Ids Tool Please Approve My Token\n Token:- "+ fkeyx)]);time.sleep(2)
...
def Create():
    ...
    def strt():
       try:
           ...
           import sys
           loop+=1
           sys.stdout.write('\r\r\033[1;37m [MALICK-XD-CREATE] \033[1;32mOK:%s \033[1;37m'%(ok));sys.stdout.flush()
           requests=r.Session()
           from fake_email import Email
           mmail=Email().Mail()
           em=mmail['mail']
           ...
           pass
    file="/sdcard/MALICK-XD/CREATE-OK.txt"
    u=5000
    clear()
    print(' Use Airplane Or Use IP Changer if ids coming cp ')
    print(' ids will be save in /sdcard/MALICK-XD/CREATE-OK.txt')
    linex()
